...
function StockCard({ group, unitLabel }: { group: ProductGroup; unitLabel: string }) {
  ...
  const statusColor = status === 'ok' ? 'var(--accent-green)'
  ...
  return (
    ...
    >
      ...
      <div className="stock-card__meta">
        <span className={`stock-card__badge stock-card__badge--${status}`}>{statusLabel}</span>
      ...
      <div className={`stock-card__expand ${hovered ? 'stock-card__expand--open' : ''}`}>
        ...
        <div className="stock-card__list">
          {group.lines.map((l) => {
            ...
            return (
              <div key={l.id} className={`stock-card__row stock-card__row--${lineStatus}`}>
